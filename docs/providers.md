...
The Codex adapter pins the upstream app-server method surface in `providers/codex/app-server-upstream-surface.json`. Update that fixture from one named OpenAI Codex commit, then run its parity test. Every request, notification, and incoming method parsed by Paseo must exist in the pinned surface or in the explicit v1 compatibility allowlist.
...
Pi control-plane RPCs wait 60 seconds by default. Override the provider's `params.rpcTimeoutMs` when extension or MCP startup on a slow host needs a different deadline. Timeout errors name the pending RPC phase and report both elapsed time and the configured deadline. This setting does not govern long-running Pi compaction or Pi extension UI results.
...
Pi model features use the optional `paseo.pi.agent-feature-host.v1` process-local contract. The generated Pi integration extension queries that host for the selected model and returns only features the active runtime can perform. It also applies changes through the same host, keeping controls such as `fast_mode` request-local instead of changing every Pi process. A missing host or unsupported model returns no feature; the app must not render a nonfunctional toggle.
